...
$pagetitle="Share Testimony || Buy at Affordable Prices in Nigeria || Bubinod Kidz Palace - Online Shopping Mall";
...
$db_images = "";
			...
			if (empty($errors)) {
				...
				$testimony = stripslashes($_REQUEST['testify']);
				$testimony = mysqli_real_escape_string($connection,$_POST['testify']);
				//$time = stripslashes($_REQUEST['time']);
				//$time = mysqli_real_escape_string($connection,$_POST['time']);
				$date = date("Y-m-d H:i:s");	
				$query1 = "INSERT INTO testimony (
						userID, testimony, time
						) VALUES (
							'{$uID}', '{$testimony}', '{$date}')";
			$result1 = mysqli_query($connection,$query1);
			if (mysqli_affected_rows($connection) == 1) {
				
				echo "<script type='text/javascript'>alert('Testimony Submitted Successfully !')</script>";
				redirect_to(SITE_PATH.'my-account');
					
				//}
				} else {
					// Display error message.
					echo "<p>Testimonial Creation failed.</p>";
	...
	<div class="main-container container banners-demo-w">
		...
		<div class="row">
			...
			<div class="banners-demo">
				<h2 class="title">Share Testimonial</h2>
					...
					<div class="buttons clearfix">
						<div class="pull-left">
							<input type="submit" name="testimony" class="btn btn-md btn-primary" value="Share Testimony">
						</div>
					</div>
					</form>
</div>
